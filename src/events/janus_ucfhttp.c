...
/* Plugin information */
#define JANUS_UCFHTTP_VERSION			1
#define JANUS_UCFHTTP_VERSION_STRING	"0.0.1"
#define JANUS_UCFHTTP_DESCRIPTION		"This is a trivial sample event handler plugin for Janus, which forwards events via HTTP POST."
#define JANUS_UCFHTTP_NAME				"JANUS UCFRAMEWORK_HTTP plugin"
#define JANUS_UCFHTTP_AUTHOR			"UC Framework"
#define JANUS_UCFHTTP_PACKAGE			"janus.eventhandler.ucfhttp"

/* Plugin methods */
janus_eventhandler *create(void);
int janus_ucf_http_init(const char *config_path);
void janus_ucf_http_destroy(void);
int janus_ucf_http_get_api_compatibility(void);
int janus_ucf_http_get_version(void);
const char *janus_ucf_http_get_version_string(void);
const char *janus_ucf_http_get_description(void);
const char *janus_ucf_http_get_name(void);
const char *janus_ucf_http_get_author(void);
const char *janus_ucf_http_get_package(void);
void janus_ucf_http_incoming_event(json_t *event);
json_t *janus_ucf_http_handle_request(json_t *request);

/* Event handler setup */
static janus_eventhandler janus_sampleevh =
	JANUS_EVENTHANDLER_INIT (
		.init = janus_ucf_http_init,
		.destroy = janus_ucf_http_destroy,

		.get_api_compatibility = janus_ucf_http_get_api_compatibility,
		.get_version = janus_ucf_http_get_version,
		.get_version_string = janus_ucf_http_get_version_string,
		.get_description = janus_ucf_http_get_description,
		.get_name = janus_ucf_http_get_name,
		.get_author = janus_ucf_http_get_author,
		.get_package = janus_ucf_http_get_package,

		.incoming_event = janus_ucf_http_incoming_event,
		.handle_request = janus_ucf_http_handle_request,
...
static void *janus_ucf_http_handler(void *data);
...
static void janus_ucf_http_event_free(json_t *event) {
...
#define JANUS_UCF_HTTP_ERROR_INVALID_REQUEST		411
#define JANUS_UCF_HTTP_ERROR_MISSING_ELEMENT		412
#define JANUS_UCFHTTP_ERROR_INVALID_ELEMENT		413
#define JANUS_UCFHTTP_ERROR_UNKNOWN_ERROR			499
...
int janus_ucf_http_init(const char *config_path) {
	...
	char filename[255];
	g_snprintf(filename, 255, "%s/%s.jcfg", config_path, JANUS_UCFHTTP_PACKAGE);
	JANUS_LOG(LOG_VERB, "Configuration file: %s\n", filename);
	janus_config *config = janus_config_parse(filename);
	if(config == NULL) {
		JANUS_LOG(LOG_WARN, "Couldn't find .jcfg configuration file (%s), trying .cfg\n", JANUS_UCFHTTP_PACKAGE);
		g_snprintf(filename, 255, "%s/%s.cfg", config_path, JANUS_UCFHTTP_PACKAGE);
		JANUS_LOG(LOG_VERB, "Configuration file: %s\n", filename);
	...
	events = g_async_queue_new_full((GDestroyNotify) janus_ucf_http_event_free);
	...
	GError *error = NULL;
	handler_thread = g_thread_try_new("janus sampleevh handler", janus_ucf_http_handler, NULL, &error);
	if(error != NULL) {
...
int janus_ucf_http_get_version(void) {
	return JANUS_UCFHTTP_VERSION;
}

const char *janus_ucf_http_get_version_string(void) {
	return JANUS_UCFHTTP_VERSION_STRING;
}

const char *janus_ucf_http_get_description(void) {
	return JANUS_UCFHTTP_DESCRIPTION;
}

const char *janus_ucf_http_get_name(void) {
	return JANUS_UCFHTTP_NAME;
}

const char *janus_ucf_http_get_author(void) {
	return JANUS_UCFHTTP_AUTHOR;
}

const char *janus_ucf_http_get_package(void) {
	return JANUS_UCFHTTP_PACKAGE;
}
...
json_t *janus_ucf_http_handle_request(json_t *request) {
	...
	JANUS_VALIDATE_JSON_OBJECT(request, request_parameters,
		...
		JANUS_UCF_HTTP_ERROR_MISSING_ELEMENT, JANUS_UCFHTTP_ERROR_INVALID_ELEMENT);
	if(error_code != 0)
	...
	if(!strcasecmp(request_text, "tweak")) {
		...
		if(req_backend && strstr(req_backend, "http") != req_backend) {
			/* Not an HTTP address */
			error_code = JANUS_UCFHTTP_ERROR_INVALID_ELEMENT;
		...
		error_code = JANUS_UCF_HTTP_ERROR_INVALID_REQUEST;
